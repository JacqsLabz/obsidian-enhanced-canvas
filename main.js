...
function getPropertyKey(settings, name) {
  const prefix = typeof settings.propertyPrefix === "string" ? settings.propertyPrefix : "";
  return `${prefix}${name}`;
}
function ensureCanvasKeyOrder(frontmatter, canvasKeys, canvasPropertyKey = "canvas") {
  ...
  reordered.splice(reordered.indexOf(keys[firstTargetIndex]), 0, canvasPropertyKey);
...
var EnhancedCanvas = class extends import_obsidian6.Plugin {
  constructor() {
    ...
    this.updateFrontmatter = async (file, link, action, propertyName) => {
      ...
      await this.app.fileManager.processFrontMatter(file, (fm) => {
        const existingValue = Reflect.get(fm, propertyKey);
        ...
        if (finalArray.length > 0) {
          if (finalArray.length === 1 && wasString) {
            Reflect.set(fm, propertyKey, finalArray[0]);
          } else {
            Reflect.set(fm, propertyKey, finalArray);
          }
        } else {
          Reflect.deleteProperty(fm, propertyKey);
        }
        ensureCanvasKeyOrder(fm, [propertyKey], getPropertyKey(this.settings, "canvas"));
      });
    };
  ...
  addProperty(node, propertyName, basename) {
    ...
    if (cached && cached[basenameKey] && frontmatterValueToArray(cached[canvasKey]).includes(canvasLink))
      return;
    void this.app.fileManager.processFrontMatter(file, (frontmatter) => {
      if (!frontmatter)
        return;
      if (!frontmatter[canvasKey]) {
        frontmatter[canvasKey] = [];
      }
      if (!frontmatter[canvasKey].includes(canvasLink)) {
        frontmatter[canvasKey].push(canvasLink);
      }
  ...
  removeProperty(node, propertyName, basename) {
    ...
    const canvasLink = `[[${propertyName}]]`;
    const cached = (_a = this.app.metadataCache.getFileCache(file)) == null ? void 0 : _a.frontmatter;
    if (!cached || !(basenameKey in cached) && !frontmatterValueToArray(cached[canvasKey]).includes(canvasLink))
      return;
    return this.app.fileManager.processFrontMatter(file, (frontmatter) => {
      ...
      if (frontmatter[canvasKey]) {
        frontmatter[canvasKey] = frontmatter[canvasKey].filter((link) => link !== canvasLink);
        if (frontmatter[canvasKey].length === 0) {
    ...
    await Promise.all(Array.from(desiredByPath.entries()).map(async ([path, desired]) => {
      ...
      try {
        await this.app.fileManager.processFrontMatter(file, (frontmatter) => {
          ...
          if (!frontmatter[canvasKey]) {
            frontmatter[canvasKey] = [];
          } else if (typeof frontmatter[canvasKey] === "string") {
            frontmatter[canvasKey] = [frontmatter[canvasKey]];
          }
          if (Array.isArray(frontmatter[canvasKey])) {
            for (const link of desired.canvasLinks) {
              if (!frontmatter[canvasKey].includes(link)) {
                frontmatter[canvasKey].push(link);
              }
            }
          }
  ...
  updatePropertyIconMarkers() {
    const keys = new Set([
      ...
      ...this.app.vault.getFiles().filter((file) => file.extension === "canvas").map((file) => getPropertyKey(this.settings, file.basename).toLowerCase())
    ]);
    activeDocument.querySelectorAll(".metadata-property[data-property-key]").forEach((property) => {
      property.classList.toggle("enhanced-canvas-property", keys.has((property.getAttribute("data-property-key") || "").toLowerCase()));
    });
  }
  checkReleaseNotes() {
    try {
  ...
  registerCanvasAutoLink() {
    const processNodeUpdate = async (e) => {
      ...
      const canvasKey = getPropertyKey(this.settings, canvasName);
      const cachedLinks = frontmatterValueToArray(
        (_f = (_e = this.app.metadataCache.getFileCache(fromFile)) == null ? void 0 : _e.frontmatter) == null ? void 0 : _f[canvasKey]
      );
      ...
      await this.app.fileManager.processFrontMatter(fromFile, (fm) => {
        const existingValue = Reflect.get(fm, canvasKey);
        const wasString = typeof existingValue === "string" && existingValue.trim() !== "";
        ...
        if (linkToAdd && !next.includes(linkToAdd))
          next.push(linkToAdd);
        if (next.length > 0) {
          Reflect.set(fm, canvasKey, next.length === 1 && wasString ? next[0] : next);
        } else {
          Reflect.deleteProperty(fm, canvasKey);
        }
        ensureCanvasKeyOrder(fm, [canvasKey], getPropertyKey(this.settings, "canvas"));
      });
    };
  ...
  async cleanupCanvasProperties() {
    const canvasBasenames = new Set(
      this.app.vault.getFiles().filter((file) => file.extension === "canvas").map((file) => getPropertyKey(this.settings, file.basename))
    );
    ...
    await Promise.all(this.app.vault.getMarkdownFiles().map(async (file) => {
      ...
      if (!cached)
        return;
      const canvasKey = getPropertyKey(this.settings, "canvas");
      if (!(canvasKey in cached) && !Object.keys(cached).some((key) => canvasBasenames.has(key)))
        return;
      try {
        await this.app.fileManager.processFrontMatter(file, (frontmatter) => {
          if (!frontmatter)
            return;
          const canvasLinks = Array.isArray(frontmatter[canvasKey]) ? frontmatter[canvasKey] : typeof frontmatter[canvasKey] === "string" ? [frontmatter[canvasKey]] : [];
          for (const link of canvasLinks) {
            if (typeof link === "string")
              delete frontmatter[getPropertyKey(this.settings, linkToBasename(link))];
          }
          delete frontmatter[canvasKey];
  ...
  async cleanupObsoletePrefixedProperties() {
    const canvasBasenames = this.app.vault.getFiles().filter((file) => file.extension === "canvas").map((file) => file.basename);
    const currentKeys = new Set(canvasBasenames.map((basename) => getPropertyKey(this.settings, basename)));
    const prefixBase = typeof this.settings.propertyPrefix === "string" ? this.settings.propertyPrefix.replace(/[-_\s]+$/, "") : "";
    await Promise.all(this.app.vault.getMarkdownFiles().map(async (file) => {
      const cached = this.app.metadataCache.getFileCache(file)?.frontmatter;
      if (!cached)
        return;
      const obsoleteKeys = Object.keys(cached).filter((key) => {
        return canvasBasenames.some((basename) => {
          if (!key.endsWith(basename) || currentKeys.has(key))
            return false;
          return key === basename || prefixBase && key.startsWith(prefixBase);
        });
      });
...
var EnhancedCanvasSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian6.Setting(containerEl).setName("Frontmatter property prefix").setDesc(
      "Prefix added to the canvas and per-canvas properties, for example 'ec-'. Leave blank to use the default names."
    ).addText(
      (text) => text.setValue(this.plugin.settings.propertyPrefix).setPlaceholder("Optional prefix").onChange(async (value) => {
        ...
        this.plugin.propertyPrefixSyncTimer = window.setTimeout(() => {
          ...
          void this.plugin.cleanupObsoletePrefixedProperties().then(() => this.plugin.syncAllCanvasProperties());
        }, 500);
      })
    );
